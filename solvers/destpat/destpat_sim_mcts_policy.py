# destpat_sim_mcts_policy.py
#
# Solve the SimDestpatEnv problem with the MCTS policy-parallel algorithm
#
# Author: Giacomo Del Rio
# Creation date: 2024 Jan 26
...
class DestpatAsyncPlotter:
    def __init__(self, config: Dict, epoch: int, plots_dir: Path, renders_dir: Path, shared_storage: ActorHandle):
        ...
        self.policy = self.policy_class(learning_rate=self.cfg['learning_rate'],
                                        obs_shape=(1, n_cells, n_cells),  # (n_cells * n_cells * 4,)
                                        # (n_cells * n_cells * 7,),  (n_cells * n_cells + n_cells * 5,),
...
def train_new():
    config = {
        # --- Environment
        'env_class': 'environments.destpat.destpat_env_sim.SimDestpatEnv',
