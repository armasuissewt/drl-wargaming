# aerialsim_plotter.py
#
# AerialsimPlotter produces a graphical representation of the simulation
#
...
from simulator.units_of_measure import DEG_TO_RAD, PI_HALF

ColorRGBA = namedtuple('ColorRGBA', ['red', 'green', 'blue', 'alpha'])

AERIAL_COLORS = {
    'red_outline': ColorRGBA(0.2, 0.0, 0.0, 1),
    'red_fill': ColorRGBA(0.8, 0.2, 0.2, 0.8),
    'blue_outline': ColorRGBA(0.0, 0.0, 0.2, 1),
    'blue_fill': ColorRGBA(0.3, 0.6, 0.9, 0.8),
    'yellow_outline': ColorRGBA(0.2, 0.2, 0.0, 1),
    'yellow_fill': ColorRGBA(0.8, 0.8, 0.2, 1.0)
}
...
class PlotConfig:
    def __init__(self):
        self.show_grid = True
        self.units_scale = 30
        self.background_color = '#e7eddf'  # '#191b24'
        self.borders_color = '#000000'  # '#ffffff'
        self.sprites_info_font = "sans-serif"
        self.sprites_info_font_style = cairo.FONT_SLANT_NORMAL
        self.sprites_info_font_size = 14
...
class Text(Drawable):
    def __init__(self, center_lat: float, center_lon: float, text: str, text_color=None, zorder: int = 0):
        """ Draw some text """
        super().__init__(zorder)
        self.center_lat = center_lat
        self.center_lon = center_lon
...
class Airplane(Sprite):
    def __init__(self, lat: float, lon: float, heading: float, edge_color='#ffffff', fill_color='#888888',
                 info_text: Optional[str] = None, cannon: Optional[Tuple[float, float]] = None,
                 radar: Optional[Tuple[float, float]] = None, zorder: int = 0):
        """ An airplane shaped sprite """
        super().__init__(lat, lon, heading, edge_color, fill_color, info_text, zorder)
        self.cannon: Optional[Tuple[float, float]] = cannon
        self.radar: Optional[Tuple[float, float]] = radar


class SamBattery(Sprite):
    def __init__(self, lat: float, lon: float, heading: float, missile_range: float, radar_range: float,
...
class SmallMissile(Sprite):
    def __init__(self, lat: float, lon: float, heading: float, edge_color='#ffffff', fill_color='#888888',
                 info_text: Optional[str] = None, zorder: int = 0):
        """ A small missile shaped sprite """
        super().__init__(lat, lon, heading, edge_color, fill_color, info_text, zorder)
...
class AerialsimPlotter:
    def __init__(self, map_extents: MapLimits, dpi=200, background_mesh: Optional[BackgroundMesh] = None,
        ...
        self.pixels_mt = self.img_width / map_extents.max_longitude_distance()
    ...
    def to_png(self, filename: Union[str, Path], objects: List[Drawable]):
        # Setup Cairo surface
        ...
        # Draw objects
        for o in sorted(objects, key=lambda x: x.zorder):
            ...
            elif isinstance(o, SmallMissile):
                self._draw_small_missile(ctx, o)
    ...
    def _draw_string(self, ctx, x, y, string):
    ...
    def _draw_text(self, ctx, o: Text):
        ctx.save()
        x, y, angle = self._get_image_xya(o.center_lat, o.center_lon, 0)
        ctx.set_source_rgba(*o.text_color)
        self._draw_string(ctx, x, y, o.text)
        ctx.new_path()
        ctx.restore()
    ...
    def _draw_airplane(self, ctx, o: Airplane):
        ...
        if o.info_text:
            ...
            self._draw_string(ctx, 0, -self.cfg.sprites_info_spacing, o.info_text)
        ...
        ctx.stroke()
        # Cannon
        if o.cannon is not None:
            ctx.set_dash([5, 5])
            ctx.move_to(0, 0)
            half_cannon_rad = o.cannon[1] * DEG_TO_RAD / 2.0
            ctx.arc(0.0, 0.0, self.pixels_mt * o.cannon[0], PI_HALF - half_cannon_rad, PI_HALF + half_cannon_rad)
            ctx.close_path()
            ctx.stroke()
        # Radar
        if o.radar is not None:
            ctx.set_dash([5, 5])
            ctx.move_to(0, 0)
            half_radar_rad = o.radar[1] * DEG_TO_RAD / 2.0
            ctx.arc(0.0, 0.0, self.pixels_mt * o.radar[0], PI_HALF - half_radar_rad, PI_HALF + half_radar_rad)
            ctx.close_path()
            ctx.stroke()
    ...
    def _draw_sam_battery(self, ctx, o: SamBattery):
        ...
        ctx.set_dash([5, 5])
        ctx.move_to(0, 0)
        ctx.arc(0.0, 0.0, self.pixels_mt * o.radar_range,
                math.pi / 2.0 - o.radar_amplitude_deg / 180 * math.pi, math.pi / 2.0)
        ctx.close_path()
        ctx.stroke()
        # Missile range
        ctx.arc(0.0, 0.0, self.pixels_mt * o.missile_range, 0, 2.0 * math.pi)
        ctx.stroke()
    ...
    def _draw_small_missile(self, ctx, o: SmallMissile):
        # Translate to waypoint position
        x, y, angle = self._get_image_xya(o.lat, o.lon, o.heading)
        ctx.save()
        ctx.translate(x, y)
        ...
        ctx.set_line_width(1)
        # Body
        ctx.move_to(0.04 * self.cfg.units_scale, -0.3 * self.cfg.units_scale)
        ctx.line_to(0.04 * self.cfg.units_scale, 0.0 * self.cfg.units_scale)
        ctx.line_to(0.0 * self.cfg.units_scale, 0.2 * self.cfg.units_scale)
        # Sym
        ctx.line_to(-0.04 * self.cfg.units_scale, 0.0 * self.cfg.units_scale)
        ctx.line_to(-0.04 * self.cfg.units_scale, -0.3 * self.cfg.units_scale)
        ctx.close_path()
        ctx.set_source_rgba(*o.fill_color)
        ctx.fill_preserve()
        ctx.set_source_rgba(*o.edge_color)
